...
export async function POST(request: Request) {
  try {
    ...
    const subject = `Nueva Tarea Asignada en AgroVista: ${task.title}`;
    const text = `Hola ${user.name},\n\nSe te ha asignado una nueva tarea en AgroVista:\n\n- Título: ${task.title}\n- Descripción: ${task.description}\n- Prioridad: ${task.priority}\n- Creada por: ${task.createdBy.name}\n\nPuedes ver más detalles en la aplicación.\n\nSaludos,\nEl equipo de AgroVista`;
    
    // Basic HTML version of the email
    const html = `
      <div style="font-family: sans-serif; line-height: 1.5;">
        <h2>Nueva Tarea Asignada en AgroVista</h2>
        <p>Hola ${user.name},</p>
        <p>Se te ha asignado una nueva tarea en la plataforma AgroVista. A continuación se muestran los detalles:</p>
