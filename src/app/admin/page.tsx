...
import { Building, ShieldCheck, ShieldAlert, Power, PowerOff, MapPin, User as UserIcon, DollarSign, Activity, Users, Clock, AlertTriangle, CheckCircle2, BellRing, Send, MessageSquare, ChevronDown, Mail, Sun, Moon, Sparkles, ThermometerSnowflake, CloudRain, Globe } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-500 pb-10">
      ...
      <Tabs defaultValue="analytics" className="w-full pb-20 md:pb-0">
        {/* Desktop Tabs Bar */}
        <TabsList className="hidden md:grid w-full grid-cols-7 max-w-[1150px] mb-6 h-auto p-1.5 bg-stone-100 dark:bg-stone-800 rounded-xl shadow-inner">
          ...
          <TabsTrigger value="web_analytics" title="Tráfico Web" className="flex items-center justify-center gap-2 py-2.5 px-2 text-xs sm:text-sm font-semibold transition-all data-[state=active]:shadow">
            <Globe className="h-4 w-4 shrink-0 text-teal-600 dark:text-teal-400" />
        ...
        <div className="md:hidden fixed bottom-5 left-1/2 -translate-x-1/2 z-50 w-[94%] max-w-[420px] bg-white/85 dark:bg-stone-900/85 backdrop-blur-xl border border-stone-200/80 dark:border-stone-800/80 rounded-full shadow-2xl shadow-stone-900/15 p-1.5 flex items-center justify-between gap-1 transition-all duration-300">
          <TabsList className="flex w-full items-center justify-around bg-transparent p-0 border-0 h-auto space-x-0">
            ...
            <TabsTrigger 
              value="web_analytics" 
              title="Tráfico Web"
              className="relative flex items-center justify-center h-10 w-10 rounded-full p-0 transition-all duration-300 border-0 bg-transparent text-stone-500 dark:text-stone-400 data-[state=active]:bg-stone-900 data-[state=active]:text-teal-400 dark:data-[state=active]:bg-white dark:data-[state=active]:text-teal-600 data-[state=active]:shadow-md data-[state=active]:scale-105"
            >
            ...
            >
              <Building className="h-4 w-4 stroke-[2.2]" />
            </TabsTrigger>
            ...
            >
              <BellRing className="h-4 w-4 stroke-[2.2]" />
            </TabsTrigger>
          </TabsList>
        </div>
        ...
        <TabsContent value="web_analytics" className="mt-0 space-y-6">
          <WebAnalyticsView />
        </TabsContent>
